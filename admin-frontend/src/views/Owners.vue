<template>
  <div class="owners-container">
      ...
      <el-table :data="ownersList" style="width: 100%; margin-top: 20px;">
        ...
        <el-table-column label="操作" width="300">
          <template #default="scope">
            ...
            <el-button type="warning" size="small" @click="editOwner(scope.row)">修改</el-button>
            <el-button 
              type="danger" 
              size="small" 
    ...
    <!-- 机主详情对话框 -->
    <el-dialog v-model="viewDialogVisible" title="机主详情" width="800px">
      <div v-if="selectedOwner">
        ...
        <div class="owner-header-info" style="display: flex; align-items: center; margin-bottom: 20px; padding: 16px; background-color: #f5f7fa; border-radius: 8px;">
          <el-avatar 
            :src="selectedOwner.avatar || '/avatar.png'" 
            size="100" 
          ...
          <div>
            ...
            <div style="margin-top: 10px; line-height: 1.8;">
              ...
              <div><strong>状态:</strong> <el-tag :type="selectedOwner.status === 1 ? 'success' : 'danger'">{{ selectedOwner.status === 1 ? '正常' : '禁用' }}</el-tag></div>
              <div><strong>审核状态:</strong> <el-tag :type="getStatusType(selectedOwner.auditStatus)">{{ getStatusText(selectedOwner.auditStatus) }}</el-tag></div>
            </div>
          </div>
        </div>
        ...
        <el-descriptions :column="2" border class="info-section" style="margin-bottom: 24px;">
          ...
          <el-descriptions-item label="审核时间">{{ selectedOwner.auditTime || '暂无' }}</el-descriptions-item>
          <el-descriptions-item label="拒绝原因">{{ selectedOwner.rejectReason || '暂无' }}</el-descriptions-item>
        </el-descriptions>
        ...
        <el-descriptions :column="2" border class="info-section" style="margin-bottom: 24px;">
          ...
          <el-descriptions-item label="信誉分">{{ selectedOwner.creditScore || 0 }}</el-descriptions-item>
        </el-descriptions>

        <!-- 资质证明部分 -->
        <h4 style="margin-top: 0; margin-bottom: 16px; color: #303133;">资质证明</h4>
        <div v-if="selectedOwner.avatar" class="license-images">
          <el-image 
            :src="selectedOwner.avatar" 
            style="width: 250px; height: 200px; margin-right: 15px; margin-bottom: 15px;"
            :preview-src-list="[selectedOwner.avatar]"
          />
        </div>
    ...
    <!-- 修改机主对话框 -->
    <el-dialog v-model="editDialogVisible" title="修改机主信息" width="500px">
      <el-form :model="editForm" label-width="80px">
        ...
        <el-form-item label="经营区域">
          <el-input v-model="editForm.commonArea"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    viewOwnerDetail(owner) {
        ...
        async saveOwner() {
          try {
            const response = await axios.put('/admin/owners', this.editForm)
        ...
        async deleteOwner(owner) {
          this.$confirm('确定要删除该机主吗？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(async () => {
            try {
              const response = await axios.delete(`/admin/owners/${owner.id}`)
              if (response.code === 200) {
        ...
        async toggleStatus(owner) {
          const newStatus = owner.status === 1 ? 0 : 1
          const action = newStatus === 1 ? '启用' : '禁用'
          
          this.$confirm(`确定要${action}该机主吗？`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(async () => {
            try {
              const response = await axios.put(`/admin/owners/${owner.id}/status`, null, {

<template>
  <div class="pilots-container">
      ...
      <el-table :data="pilotsList" style="width: 100%; margin-top: 20px;">
        ...
        <el-table-column label="操作" width="300">
          <template #default="scope">
            ...
            <el-button type="warning" size="small" @click="editPilot(scope.row)">修改</el-button>
            <el-button 
              type="danger" 
              size="small" 
              @click="deletePilot(scope.row)"
    ...
    <!-- 飞手详情对话框 -->
    <el-dialog v-model="viewDialogVisible" title="飞手详情" width="800px">
      <el-descriptions :column="2" v-if="selectedPilot">
        ...
        <el-descriptions-item label="状态">
          <el-tag :type="selectedPilot.status === 1 ? 'success' : 'danger'">
            {{ selectedPilot.status === 1 ? '正常' : '禁用' }}
          </el-tag>
    ...
    <!-- 修改飞手对话框 -->
    <el-dialog v-model="editDialogVisible" title="修改飞手信息" width="500px">
      <el-form :model="editForm" label-width="80px">
        ...
        <el-form-item label="亩单价">
          <el-input v-model="editForm.pricePerAcre"></el-input>
        </el-form-item>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="savePilot">保存</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    editPilot(pilot) {
      this.editForm = {
        userId: pilot.userId,
        flyerId: pilot.flyerId,
        userName: pilot.userName,
        phone: pilot.phone,
        skillLevel: pilot.skillLevel,
        pricePerAcre: pilot.pricePerAcre,
        location: pilot.location,
        status: pilot.status || 1
      }
      this.editDialogVisible = true
    },
    async savePilot() {
      try {
        const response = await axios.put('/admin/flyers', this.editForm)
        if (response.code === 200) {
    ...
    async deletePilot(pilot) {
      this.$confirm('确定要删除该飞手吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const response = await axios.delete(`/admin/flyers/${pilot.flyerId}`)
          if (response.code === 200) {
    ...
    async toggleStatus(pilot) {
      const newStatus = pilot.status === 1 ? 0 : 1
      const action = newStatus === 1 ? '启用' : '禁用'
      
      this.$confirm(`确定要${action}该飞手吗？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        try {
          const response = await axios.put(`/admin/flyers/${pilot.flyerId}/status`, null, {

...
) ENGINE = InnoDB AUTO_INCREMENT = 27 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '喷洒需求表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备动态记录表' ROW_FORMAT = DYNAMIC;
...
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '评价记录表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB AUTO_INCREMENT = 34 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '用户消息表' ROW_FORMAT = DYNAMIC;
...
CREATE TABLE `payment_log`  (
  ...
  `order_id` varchar(50) CHARACTER SET armscii8 COLLATE armscii8_general_ci NOT NULL,
  `order_type` varchar(20) CHARACTER SET armscii8 COLLATE armscii8_general_ci NOT NULL,
  `amount` decimal(10, 2) NOT NULL,
  `status` varchar(20) CHARACTER SET armscii8 COLLATE armscii8_general_ci NOT NULL,
  `payment_method` varchar(20) CHARACTER SET armscii8 COLLATE armscii8_general_ci NOT NULL,
  `transaction_id` varchar(100) CHARACTER SET armscii8 COLLATE armscii8_general_ci NULL DEFAULT NULL,
  `create_time` datetime NOT NULL,
  `remark` varchar(200) CHARACTER SET armscii8 COLLATE armscii8_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `idx_order_id`(`order_id` ASC) USING BTREE,
  INDEX `idx_user_id`(`user_id` ASC) USING BTREE,
  INDEX `idx_create_time`(`create_time` ASC) USING BTREE,
  CONSTRAINT `payment_log_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `sys_user` (`user_id`) ON DELETE RESTRICT ON UPDATE RESTRICT
...
CREATE TABLE `payment_order`  (
  ...
  `amount` decimal(10, 2) NOT NULL,
  `payment_method` varchar(20) CHARACTER SET armscii8 COLLATE armscii8_general_ci NOT NULL,
  `out_trade_no` varchar(100) CHARACTER SET armscii8 COLLATE armscii8_general_ci NOT NULL,
  `status` varchar(20) CHARACTER SET armscii8 COLLATE armscii8_general_ci NOT NULL,
  `create_time` datetime NOT NULL,
  ...
  INDEX `idx_user_id`(`user_id` ASC) USING BTREE,
  CONSTRAINT `payment_order_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `sys_user` (`user_id`) ON DELETE RESTRICT ON UPDATE RESTRICT
) ENGINE = InnoDB CHARACTER SET = armscii8 COLLATE = armscii8_general_ci ROW_FORMAT = Dynamic;
...
CREATE TABLE `sys_user`  (
  ...
  `username` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '用户名（登录用，唯一，小程序登录时存储openid）',
  `password` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '密码（BCrypt加密）',
  `real_name` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '真实姓名',
  `phone` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '手机号（小程序用户可为空）',
  `wx_openid` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '微信OpenID（小程序登录专用）',
  `role_type` int NOT NULL DEFAULT 0 COMMENT '角色类型：0-未选择，1-农户，2-飞手，3-机主',
  `audit_status` int NULL DEFAULT 1 COMMENT '审核状态：0-待审核，1-通过，2-拒绝（仅飞手/机主有值）',
...
CREATE TABLE `t_device_operate_log`  (
  ...
  INDEX `idx_device_id`(`device_id` ASC) USING BTREE,
  INDEX `idx_operator_id`(`operator_id` ASC) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备操作日志表' ROW_FORMAT = Dynamic;
...
) ENGINE = InnoDB AUTO_INCREMENT = 20 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '用户交易记录表（收支明细）' ROW_FORMAT = DYNAMIC;
...
-- ----------------------------
-- Table structure for ai_diagnosis_history
-- ----------------------------
DROP TABLE IF EXISTS `ai_diagnosis_history`;
CREATE TABLE `ai_diagnosis_history`  (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '诊断记录ID（主键）',
  `user_id` bigint NOT NULL COMMENT '农户用户ID（关联sys_user.user_id）',
  `crop_type` varchar(50) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '作物类型',
  `image_url` varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '诊断图片URL',
  `disease_name` varchar(100) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NOT NULL COMMENT '病虫害名称',
  `confidence` int NOT NULL COMMENT '置信度（0-100）',
  `recommended_pesticide` varchar(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '推荐农药',
  `control_measures` varchar(2000) CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci NULL DEFAULT NULL COMMENT '防治措施',
  `create_time` datetime NULL DEFAULT CURRENT_TIMESTAMP COMMENT '诊断时间',
  PRIMARY KEY (`id`) USING BTREE,
  INDEX `idx_user_id`(`user_id` ASC) USING BTREE,
  INDEX `idx_create_time`(`create_time` DESC) USING BTREE,
  CONSTRAINT `fk_diagnosis_user` FOREIGN KEY (`user_id`) REFERENCES `sys_user` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'AI诊断历史记录表' ROW_FORMAT = Dynamic;

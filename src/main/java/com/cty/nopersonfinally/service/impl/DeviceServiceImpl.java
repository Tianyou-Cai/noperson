...
public class DeviceServiceImpl extends ServiceImpl<DeviceMapper, DroneDevice> implements DeviceService {
    ...
    public int bindDeviceToFlyer(Long deviceId, Long flyerId) {
        ...
        System.out.println("开始更新设备状态: deviceId=" + deviceId + ", flyerId=" + flyerId + ", rentalStatus=1");
        boolean result = update(updateWrapper);
        System.out.println("设备状态更新结果: " + result);
        
        // 查询更新后的设备状态
        DroneDevice updatedDevice = getById(deviceId);
        System.out.println("更新后设备状态: flyerId=" + updatedDevice.getFlyerId() + ", rentalStatus=" + updatedDevice.getRentalStatus());
    ...
    public boolean rentDevice(Long deviceId, Long flyerId) {
        ...
        long lockExpireTime = 10;
        TimeUnit lockTimeUnit = TimeUnit.SECONDS;

        System.out.println("====== 开始执行设备租借操作 ======");
        System.out.println("设备ID=" + deviceId + ", 飞手ID=" + flyerId);

        // 尝试获取分布式锁
        System.out.println("0. 尝试获取分布式锁...");
        boolean lockAcquired = distributedLock.tryLockWithRetry(lockKey, lockValue, lockExpireTime, 3, 100);
        if (!lockAcquired) {
            System.out.println("获取分布式锁失败，设备正在被其他操作占用");
            throw new RuntimeException("设备正在被其他操作占用，请稍后重试");
        }
        System.out.println("成功获取分布式锁，锁Key=" + lockKey + "，锁Value=" + lockValue);
        ...
        try {
            ...
            // 计算需要扣除的金额（租金 + 押金）
            double rentPrice = device.getHourlyRent() != null ? device.getHourlyRent().doubleValue() : 0.0;
            double totalAmount = rentPrice;
            
            // 如果好评率<=50，需要扣除500押金
            if (positiveRate <= 50) {
                totalAmount += 500.0;
                System.out.println("飞手好评率<=50，需要扣除500押金，总计：" + totalAmount);
            } else {
                System.out.println("飞手好评率>50，无需押金，总计：" + totalAmount);
            }
            
            // 4. 扣除用户余额
            if (totalAmount > 0) {
                System.out.println("4. 扣除用户余额：" + totalAmount);
                try {
                    userInfoService.updateUserBalance(flyerId, -totalAmount);
                    System.out.println("余额扣除成功");
                } catch (RuntimeException e) {
                    System.out.println("余额扣除失败：" + e.getMessage());
                    throw e; // 抛出异常以触发事务回滚
                }
            }

            // 5. 更新设备信息，使用乐观锁防止并发问题
            System.out.println("5. 更新设备信息，绑定飞手并设置租借状态...");
            DroneDevice updateEntity = new DroneDevice();
            updateEntity.setDeviceId(deviceId);
            ...
            // 释放分布式锁
            System.out.println("释放分布式锁，lockKey=" + lockKey + "，lockValue=" + lockValue);
    ...
    public boolean returnDevice(Long deviceId, Long flyerId) {
        ...
        // 4. 检查飞手好评率，判断是否需要退还押金
        System.out.println("4. 检查飞手好评率，判断是否需要退还押金...");
        Double positiveRateObj = flyerEvaluationService.getPositiveRate(flyerId);
        double positiveRate = positiveRateObj != null ? positiveRateObj : 0.0;
        System.out.println("飞手好评率：" + positiveRate);
        ...
        if (!updateResult) {
            System.out.println("归还失败：设备状态更新失败");
    ...
    public DeviceDetailVO getDeviceDetail(Long deviceId) {
        ...
        // 添加null检查，避免设备状态为null时的异常
            Integer deviceStatus = device.getStatus();
            String statusDesc = "未知";
            if (deviceStatus != null) {
                DeviceStatusEnum statusEnum = DeviceStatusEnum.getByCode(deviceStatus);
                if (statusEnum != null) {
                    statusDesc = statusEnum.getDesc();
                }
            }
            basicInfo.setStatusDesc(statusDesc);
        ...
        // 设置每小时租金（将BigDecimal转换为Double）
        basicInfo.setHourlyRent(device.getHourlyRent() != null ? device.getHourlyRent().doubleValue() : null);
    ...
    public IPage<DeviceDetailVO> getAllDevices(int pageNum, int pageSize, Long ownerId, Integer status) {
        ...
        IPage<DeviceDetailVO> resultPage = devicePage.convert(device -> {
            ...
            basicInfo.setMaxLoad(device.getMaxLoad());
            // 设置每小时租金
            if (device.getHourlyRent() != null) {
                basicInfo.setHourlyRent(device.getHourlyRent().doubleValue());
            }
            ...
            if (device.getOwnerId() != null) {
                ...
                // 查询sys_user表获取机主信息（real_name和phone）
                SysUser sysUser = sysUserMapper.selectById(device.getOwnerId());
                if (sysUser != null) {
                    // 设置机主姓名（从sys_user表的real_name字段获取）
                    ownerInfo.setRealName(sysUser.getRealName());
                    // 设置机主电话
                    ownerInfo.setPhone(sysUser.getPhone());
                }
    ...
    public boolean deleteDevice(Long deviceId) {
        ...
        // 执行删除操作
        boolean result = deviceMapper.deleteDeviceById(deviceId);
        
        if (result) {
            // 更新机主的设备总数
            updateOwnerDeviceCount(device.getOwnerId());
        }
     ...
     * @param ownerId 机主ID
     */
    private void updateOwnerDeviceCount(Long ownerId) {
        // 查询机主的设备总数
        QueryWrapper<DroneDevice> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("owner_id", ownerId)
                .eq("is_deleted", 0);
        int deviceCount = (int) count(queryWrapper);
        
        // 更新user_owner表的设备总数
        UserOwner userOwner = userOwnerMapper.selectById(ownerId);
        if (userOwner != null) {
            userOwner.setDeviceTotal(deviceCount);
            userOwnerMapper.updateById(userOwner);
        }
    }
    ...
    @Override
    @Transactional
    public boolean save(DroneDevice device) {
        boolean result = super.save(device);
        if (result && device.getOwnerId() != null) {
            // 更新机主的设备总数
            updateOwnerDeviceCount(device.getOwnerId());
        }
        return result;

...
public interface DemandMapper extends BaseMapper<OrderDemand> {
     ...
     * @return 超时订单列表
     */
    @Select("SELECT * FROM demand WHERE status = 0 AND payment_status = 0 AND create_time < #{timeoutTime}")
    List<OrderDemand> selectTimeoutPendingPayOrders(@Param("timeoutTime") LocalDateTime timeoutTime);

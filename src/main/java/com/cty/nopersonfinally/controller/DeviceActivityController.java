...
public class DeviceActivityController {
    ...
    public Result<IPage<DeviceActivity>> getRecentActivities(
        ...
        IPage<DeviceActivity> page = deviceActivityService.getRecentActivities(pageNum, pageSize, deviceId, activityTypes, userId);
        return Result.ok(page);
    }

    /**
     * 获取指定设备的最近动态（只显示当前机主的设备）
     */
    ...
    public Result<List<DeviceActivity>> getDeviceActivities(
        ...
        // 获取当前登录用户ID
        Long userId = getCurrentUserId();
        List<DeviceActivity> activities = deviceActivityService.getDeviceRecentActivities(deviceId, limit, userId);

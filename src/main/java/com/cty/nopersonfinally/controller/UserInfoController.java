...
import com.cty.nopersonfinally.pojo.vo.WalletBalanceVO;
...
public class UserInfoController {
    ...
    public Result<?> getWalletBalance(@RequestHeader("Authorization") String token) {
        // 从token中获取用户ID
        Long userId = jwtUtil.getUserIdFromToken(token);
        if (userId == null) {
            return Result.error("无效的token");
        ...
        Double balance = userInfoService.getUserBalance(userId);
        return Result.ok(new WalletBalanceVO(balance));
    }

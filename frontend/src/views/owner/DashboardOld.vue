    ...
    <div class="chart-container">
      ...
      <el-card class="card chart-card status-card">
        ...
        <div class="audit-status">
          <div class="status-container" :class="`status-${parseInt(auditStatus)}`">
            <div class="status-icon">
              {{ parseInt(auditStatus) === 0 ? '⏳' : parseInt(auditStatus) === 1 ? '✅' : '❌' }}
            </div>
            <h4 class="status-title">{{ auditStatusText }}</h4>
            <p class="status-description">{{ auditStatusDesc }}</p>
            <template v-if="parseInt(auditStatus) === 0 || parseInt(auditStatus) === 2">
              <el-button 
                type="primary" 
                @click="$router.push('/qualification')"
                class="status-button"
                :class="{ 'secondary': parseInt(auditStatus) === 2 }"
              >
                {{ parseInt(auditStatus) === 0 ? '查看审核进度' : '重新提交资料' }}
...
export default {
  ...
  setup() {
    ...
    const auditStatus = ref(null) // 0待审核，1通过，2拒绝
    ...
    const auditStatusIcon = computed(() => {
      console.log('auditStatusIcon计算属性，auditStatus.value:', auditStatus.value)
      const status = parseInt(auditStatus.value)
      switch (status) {
        case 0: return 'info'
    ...
    const auditStatusText = computed(() => {
      console.log('auditStatusText计算属性，auditStatus.value:', auditStatus.value)
      console.log('auditStatus.value类型:', typeof auditStatus.value)
      const status = parseInt(auditStatus.value)
      switch (status) {
        case 0: return '待审核'
        case 1: return '审核通过'
    ...
    const fetchOwnerDetail = async () => {
      try {
        ...
        console.log('获取机主详情响应:', response)
        console.log('响应数据:', response.data)
        console.log('response.data.auditStatus:', response.data.auditStatus)
        
        // 处理auditStatus为null的情况，默认设置为0（待审核）
        auditStatus.value = response.data.auditStatus !== null ? response.data.auditStatus : 0
        console.log('设置auditStatus.value后:', auditStatus.value)
      } catch (error) {

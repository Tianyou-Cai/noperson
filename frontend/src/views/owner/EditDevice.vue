      ...
      try {
        ...
        // 调用设备详情API获取实际数据
         ...
         this.deviceForm = {
           ...
           // 每小时租金，兼容多种字段名
           hourlyRent: deviceInfo.hourlyRent ?? deviceInfo.hourly_rent ?? null
         }
        ...
        if (valid) {
          ...
          try {
            ...
            const deviceData = {
              ...
              // 后端使用 LocalDateTime，拼接上固定时间部分，避免解析错误
              purchaseTime: this.deviceForm.purchaseDate ? `${this.deviceForm.purchaseDate}T00:00:00` : null,
              // 添加picture字段，保留现有的图片路径
              picture: this.deviceForm.imageUrl || '',
              // 每小时租金
              hourlyRent: this.deviceForm.hourlyRent
            };

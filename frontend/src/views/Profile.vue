<template>
  <div class="profile-container">
    ...
    <el-card class="profile-card">
      ...
      <!-- 资质照片展示 -->
      <div v-if="ownerInfo.avatar" class="info-item">
        <span class="info-label">资质照片：</span>
        <div class="license-photos">
          <div class="license-photo-item">
            <el-image :src="ownerInfo.avatar" fit="cover" :preview-src-list="[ownerInfo.avatar]"></el-image>
          </div>
        </div>
      </div>
    </el-card>
    ...
    >
      ...
      >
        ...
        <el-form-item label="资质照片">
          <div v-if="qualificationForm.avatar" class="license-photos">
            <div class="license-photo-item">
              <el-image :src="qualificationForm.avatar" fit="cover" :preview-src-list="[qualificationForm.avatar]"></el-image>
            </div>
          </div>
          <el-upload
  ...
  setup() {
    ...
    const showQualificationEdit = () => {
      ...
      qualificationForm.value = {
        ...
        avatar: ownerInfo.value.avatar || ''
      }
      
      // 重置文件列表
      licenseFiles.value = ownerInfo.value.avatar ? [{
        name: ownerInfo.value.avatar.split('/').pop(),
        url: ownerInfo.value.avatar
      }] : []
    ...
    const handleLicenseFileSuccess = (response, uploadFile) => {
      if (response.code === 200) {
        qualificationForm.value.avatar = response.data
        ElMessage.success('上传成功')
      } else {
        ElMessage.error('上传失败，请重试')
      }
    ...
    const handleLicenseFileRemove = (file) => {
      qualificationForm.value.avatar = ''
    }
    ...
    const handleQualificationSubmit = async () => {
      ...
      try {
        ...
        const submitData = {
          ...
          avatar: qualificationForm.value.avatar,

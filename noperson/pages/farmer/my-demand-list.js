...
Page({
  ...
  createReview: function(e) {
    ...
    // 直接跳转到详情页面并显示评价表单
    wx.navigateTo({
      url: `/pages/farmer/demand-detail?id=${id}&showEvaluation=true`
    });
  },
  ...
  // 支付需求
  payDemand: function(e) {
    const id = e.currentTarget.dataset.id;
    const amount = e.currentTarget.dataset.amount;
    const orderType = e.currentTarget.dataset.orderType;
    
    let orderTypeDesc = 'INSPECTION';
    if (orderType === 1 || orderType === '1') {
      orderTypeDesc = 'SPRAY';
    }
    
    wx.navigateTo({
      url: `/pages/payment/create?orderType=${orderTypeDesc}&orderId=${id}&amount=${amount}`
    });
  }
});

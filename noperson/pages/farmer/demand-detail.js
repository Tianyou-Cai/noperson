...
Page({
  ...
  loadDemandDetail: async function() {
    ...
    try {
      ...
      // 从响应对象中获取data字段
      let formattedData = this.formatDemandDetail(demandDetailData.data);
  ...
  formatDemandDetail: function(data) {
    ...
    return {
      ...
      // 飞手信息（如果已接单）
        ...
        flyerName: data.flyerName || data.flyer_name || '',
        flyerPhone: data.flyerPhone || data.flyer_phone || '',
        flyerInfo: data.flyer_id ? {
          name: data.flyerName || data.flyer_name || '未知飞手',
          avatar: data.flyerAvatar || data.flyer_avatar || '/assets/images/user-avatar.png',
          phone: data.flyerPhone || data.flyer_phone || '',
          creditScore: data.flyerCreditScore || data.flyer_credit_score || 0,
          skillLevel: data.flyerSkillLevel || data.flyer_skill_level || '初级',
          positiveRate: data.flyerPositiveRate || data.flyer_positive_rate || 0,
          totalOrders: data.flyerTotalOrders || data.flyer_total_orders || 0
        } : null,
      // 设备信息
  ...
  showContactOptions: function() {
    const flyerPhone = (this.data.demandDetail?.flyerPhone || this.data.demandDetail?.flyer_phone || 
                       this.data.cachedDetail?.flyerPhone || this.data.cachedDetail?.flyer_phone);
    const flyerName = (this.data.demandDetail?.flyerName || this.data.demandDetail?.flyer_name || 
                      this.data.cachedDetail?.flyerName || this.data.cachedDetail?.flyer_name || '飞手');
    ...
    wx.showActionSheet({
      ...
      success: (res) => {
        if (res.tapIndex === 0) {
          // 在线联系（跳转到聊天页面）
          const flyerId = this.data.demandDetail?.flyer_id || this.data.cachedDetail?.flyer_id;
          if (flyerId) {
            // 创建会话ID：farmer_用户ID_flyer_飞手ID
            const userId = wx.getStorageSync('userInfo')?.id;
            if (userId) {
              const conversationId = `farmer_${userId}_flyer_${flyerId}`;
              wx.navigateTo({
                url: `/pages/farmer/chat/detail?conversationId=${conversationId}&otherUserId=${flyerId}&otherUserName=${encodeURIComponent(flyerName)}&otherUserType=2`
              });
            } else {
              wx.showToast({
                title: '请先登录',
  ...
  uploadImages: function(tempFilePaths) {
    ...
    const uploadPromises = tempFilePaths.map((filePath, index) => {
      return new Promise((resolve, reject) => {
        ...
        wx.uploadFile({
          url: 'http://localhost:8082/api/upload/image',
          filePath: filePath,
          name: 'file',
          success: (res) => {

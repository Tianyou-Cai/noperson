...
// 后端API基础URL
const BASE_URL = 'http://localhost:8082/api'; // 请替换为实际的后端API地址
...
const request = (url, method, data = {}, needToken = true) => {
  return new Promise((resolve, reject) => {
    ...
    if (needToken) {
      ...
      } else if (url !== '/auth/login' && url !== '/auth/register' && url !== '/mini/login') {
...
const api = {
  ...
  // 微信小程序登录
  wxMiniLogin: (data) => request('/mini/login', 'POST', data, false),
  // 选择角色
  selectRole: (data) => request('/mini/select-role', 'POST', data),
  // 获取用户角色列表
  getUserRoles: (userId) => request(`/mini/user-roles/${userId}`, 'GET'),
  // 获取可选角色
  getAvailableRoles: () => request('/mini/available-roles', 'GET'),
  // 发送验证码，参数作为URL查询参数
  ...
  getUserInfoById: (userId) => request(`/user/info/${userId}`, 'GET'),
  ...
  getWalletBalance: () => request('/user/wallet/balance', 'GET'),
  ...
  // 支付宝沙箱支付接口
  createAlipayOrder: (data) => request('/payment/alipay/create', 'POST', data),
  alipayNotify: (data) => request('/payment/alipay/notify', 'POST', data),
  ...
  // 根据机主ID获取设备列表
  getOwnerDevices: (ownerId) => {
    if (!ownerId && ownerId !== 0) {
      console.error('获取机主设备列表失败：机主ID不能为空');
      return Promise.reject(new Error('获取机主设备列表失败：机主ID不能为空'));
    }
    const formattedOwnerId = String(ownerId).trim();
    return request(`/device/owner/${formattedOwnerId}`, 'GET');
  },
  ...
  // 飞手租借机器接口（兼容两种调用方式）
  rentDeviceByFlyer: async (param) => {
    console.log('开始处理设备租借请求', param);
    
    let rentData;
    // 兼容两种调用方式：单个设备ID或包含多个参数的对象
    if (typeof param === 'string' || typeof param === 'number') {
      // 单个设备ID调用（设备详情页使用）
      rentData = {
        deviceId: param,
        rentalHours: 1, // 默认租赁1小时
        pickupTime: new Date().toISOString(), // 默认当前时间
        remark: ''
      };
    } else {
      // 对象参数调用（设备租赁页使用）
    ...
    // 严格检查设备ID
    if (!rentData || !rentData.deviceId) {
      console.error('租借机器失败：设备ID为空或无效');
      return Promise.reject(new Error('租借机器失败：设备ID不能为空'));
    }
    
    // 格式化设备ID
    const formattedDeviceId = String(rentData.deviceId).trim();
    ...
    const requestData = {
      ...
      pickupTime: rentData.pickupTime || new Date().toISOString(),
      rentalHours: rentData.rentalHours || 1,
      remark: rentData.remark || ''
    };
    console.log('租借请求体参数:', requestData);
